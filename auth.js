...
async function checkExistingSession() {
  ...
  // Handle OAuth / email confirmation redirect (arrives with ?code=...)
...
async function handleSignup(e) {
  ...
  // Supabase doesn't expose this directly, so we try signing in with a
  // dummy password — if the error is "Invalid login credentials" the
  // email EXISTS (wrong password), vs "User not found" which means new.
  const { error: checkError } = await window._supabase.auth.signInWithPassword({
    email,
    password: '##CHECK_ONLY##'
  });
  ...
  if (checkError) {
    ...
    // Any other error that isn't "user not found" → email likely exists
    ...
    // "user not found" → email is free, proceed with signup
  }

  const { error: signupError } = await window._supabase.auth.signUp({
    email,
    password,
    options: {
      data: { full_name: name },
  ...
  if (signupError) {
    showMessage(friendlyError(signupError), 'error');
    btn.disabled    = false;
    btn.textContent = 'Create Account';
  } else {
    showMessage(
